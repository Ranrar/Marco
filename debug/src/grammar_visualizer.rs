...
/// Write Markdown version of the grammar tree
fn write_markdown_grammar_tree(tree_output: &str, rule_count: usize) {
    let markdown_content = format!(
        r#"# Marco Grammar Dependency Tree

Generated automatically from `marco.pest` grammar file  
...
- **Entry Points:** `file` → `document` → `block`/`inline`
- **Total Rules:** {}
- **Key Categories:**
  - Block-level elements (paragraphs, lists, tables, admonitions)
  - Inline elements (emphasis, links, code, math)
  - Marco extensions (tabs, user mentions, bookmarks)
  - Text processing (words, symbols, formatting)

---
*Generated by Marco Grammar Analysis Tool*
"#,
        rule_count,
    ...
    match fs::write("src/results/grammar_tree.md", markdown_content) {
        Ok(_) => println!("📄 Markdown version written to src/results/grammar_tree.md"),
        Err(e) => eprintln!("❌ Failed to write Markdown file: {}", e),
    }
}

/// Write plain text version of the grammar tree
fn write_text_grammar_tree(tree_output: &str, rule_count: usize) {
...
"#,
    ...
    match fs::write("src/results/grammar_tree.txt", text_content) {
        Ok(_) => println!("📄 Plain text version written to src/results/grammar_tree.txt"),
        Err(e) => eprintln!("❌ Failed to write text file: {}", e),
    }
}
