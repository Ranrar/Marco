//! Toolbar highlight toggle helpers for Markdown mark syntax (`==...==`).
//! Behavior is validated against project-supported mark/highlight parsing.

use gtk4::prelude::*;

pub fn connect_highlight_toolbar_action(
    toolbar: &gtk4::Box,
    ...
    if let Some(button) = find_button_by_css_class(
        ...
        button.connect_clicked(move |_| {
            ...
            toggle_highlight_for_selection_or_word(editor_buffer.upcast_ref::<gtk4::TextBuffer>());
...
pub fn toggle_highlight_for_selection_or_word(text_buffer: &gtk4::TextBuffer) {
    if let Some((mut selection_start, mut selection_end)) = text_buffer.selection_bounds() {
        if selection_start.offset() != selection_end.offset() {
            maybe_expand_range_to_surrounding_highlight_markers(
                text_buffer,
                &mut selection_start,
                &mut selection_end,
            );
            toggle_highlight_on_range(text_buffer, &mut selection_start, &mut selection_end);
            return;
        }
    }
    ...
    let Some((word_start, word_end)) = find_word_bounds(&line_text, cursor_col) else {
        // Cursor might be on/adjacent to existing highlight markers — try to unwrap.
        if let Some((span_start, span_end)) = find_highlight_span_at_cursor(&line_text, cursor_col)
        {
            if let (Some(mut si), Some(mut ei)) = (
                ...
                toggle_highlight_on_range(text_buffer, &mut si, &mut ei);
                return;
            }
        }
        insert_empty_highlight_delimiters(text_buffer);
        return;
    };
    ...
    toggle_highlight_on_range(text_buffer, &mut start_iter, &mut end_iter);
}

/// Insert `====` at the current cursor position and place the cursor between
/// the two marker pairs for immediate typing.
fn insert_empty_highlight_delimiters(text_buffer: &gtk4::TextBuffer) {
...
fn maybe_expand_range_to_surrounding_highlight_markers(
    ...
    let Some((expanded_start, expanded_end)) =
        expand_to_surrounding_highlight_markers(&line_text, start_col, end_col)
    else {
        return;
    };
...
fn expand_to_surrounding_highlight_markers(
    line_text: &str,
    start_col: usize,
    end_col: usize,
) -> Option<(usize, usize)> {
    ...
    // Fallback: selection might be inside a larger highlight span.
...
fn find_highlight_span_at_cursor(line_text: &str, cursor_col: usize) -> Option<(usize, usize)> {
    let chars: Vec<char> = line_text.chars().collect();
    if chars.is_empty() {
        return None;
    }
...
fn toggle_highlight_on_range(
    text_buffer: &gtk4::TextBuffer,
    start_iter: &mut gtk4::TextIter,
    end_iter: &mut gtk4::TextIter,
) {
    let selected = text_buffer.text(start_iter, end_iter, false).to_string();
    let toggled = toggle_selected_highlight_text(&selected);
...
fn is_wrapped_with_highlight_markers(text: &str) -> bool {
    text.starts_with("==") && text.ends_with("==") && text.len() >= 4
}

fn is_wrapped_with_single_highlight_span(text: &str) -> bool {
    if !is_wrapped_with_highlight_markers(text) {
        return false;
    }
...
fn toggle_selected_highlight_text(selected: &str) -> String {
    if let Some(stripped_multi) = strip_highlight_from_each_non_whitespace_token(selected) {
        stripped_multi
    } else if is_wrapped_with_highlight_markers(selected) {
...
fn strip_highlight_from_each_non_whitespace_token(text: &str) -> Option<String> {
    ...
    while let Some((idx, ch)) = chars.peek().copied() {
        ...
        if !is_wrapped_with_single_highlight_span(token) {
            return None;
        }
...
mod tests {
    ...
    #[test]
    fn smoke_test_detects_highlight_wrapped_text() {
        assert!(is_wrapped_with_highlight_markers("==hello=="));
        assert!(!is_wrapped_with_highlight_markers("hello"));
    }
    ...
    #[test]
    fn smoke_test_expand_to_surrounding_highlight_markers_detects_wrapped_word() {
        let expanded = expand_to_surrounding_highlight_markers("==world==", 2, 7);
        assert_eq!(expanded, Some((0, 9)));
    }

    #[test]
    fn smoke_test_toggle_selected_highlight_text_for_multiple_tokens() {
        let toggled = toggle_selected_highlight_text("==hello== ==world==");
        assert_eq!(toggled, "hello world");
    }

    #[test]
    fn smoke_test_toggle_selected_highlight_text_for_single_span() {
        let toggled = toggle_selected_highlight_text("==hello world==");
        assert_eq!(toggled, "hello world");
    }

    // Extension traceability:
    // - core/src/parser/inlines/marco_mark_parser.rs (`==hi==`)
    // - core/src/parser/inlines/mod.rs sample includes `==mark==`
    #[test]
    fn smoke_test_toggle_selected_highlight_text_adds_around_bold_content() {
        let toggled = toggle_selected_highlight_text("**hello world**");
        assert_eq!(toggled, "==**hello world**==");
    }

    #[test]
    fn smoke_test_find_highlight_span_cursor_on_opening_marker() {
        assert_eq!(find_highlight_span_at_cursor("==hello==", 0), Some((0, 9)));
    }

    #[test]
    fn smoke_test_find_highlight_span_cursor_on_closing_marker() {
        assert_eq!(find_highlight_span_at_cursor("==hello==", 8), Some((0, 9)));
    }

    #[test]
    fn smoke_test_find_highlight_span_cursor_past_end() {
        assert_eq!(find_highlight_span_at_cursor("==hello==", 9), Some((0, 9)));
    }

    #[test]
    fn smoke_test_find_highlight_span_no_markers() {
        assert_eq!(find_highlight_span_at_cursor("hello", 2), None);
    }

    #[test]
    fn smoke_test_find_highlight_span_between_two_spans() {
        assert_eq!(find_highlight_span_at_cursor("==foo== ==bar==", 7), None);
    }
}
