...
/// `MainContext::find_source_by_id` returns `None` for a source that has already
/// fired or been removed, so this only calls `remove()` on one still pending.
pub fn safe_source_remove(source_id: gtk4::glib::SourceId) {
    if gtk4::glib::MainContext::default()
        .find_source_by_id(&source_id)
        .is_some()
    {
        source_id.remove();
    } else {
        log::trace!("Source {:?} already removed/fired; skipping", source_id);
    }
}
