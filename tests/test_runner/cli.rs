...
use crate::spec::{TestSpec, TestResult};
use crate::runner::{TestRunner, RunnerConfig};
use crate::diff::{DiffConfig, create_side_by_side_diff, calculate_diff_stats};
...
impl Cli {
    ...
    pub fn run(&self) -> Result<()> {
        ...
        match &self.command {
            Commands::String { markdown, expected, side_by_side } => {
                ...
                self.run_string_mode(&runner, &markdown_content, expected.as_deref(), *side_by_side)
            }
            
            Commands::File { input, output, expected } => {
                self.run_file_mode(&runner, input, output.as_ref(), expected.as_ref())
            }
            
            Commands::Html { html, spec, example } => {
                self.run_html_mode(&runner, html, spec.as_ref(), *example)
            }
            
            Commands::Spec { file, section, example, fail_fast } => {
                self.run_spec_mode(&runner, file.as_ref(), section.as_deref(), *example, *fail_fast)
            }
            
            Commands::Interactive => {
                self.run_interactive_mode(&runner)
            }
            
            Commands::Stats { spec } => {
                self.run_stats_mode(spec.as_ref())
            }
        }
    }
    
    /// Run string testing mode
    fn run_string_mode(&self, runner: &TestRunner, markdown: &str, expected: Option<&str>, side_by_side: bool) -> Result<()> {
        ...
        match result {
            ...
            TestResult::Failed { expected, actual, diff } => {
                ...
                if side_by_side {
                    let side_diff = create_side_by_side_diff(&expected, &actual, &runner.config.diff_config);
                    println!("{}", side_diff);
                } else {
                    println!("{}", diff);
                }
            ...
            TestResult::NoBaseline { actual } => {
                println!("{}", "No expected result provided. Here's the actual output:".yellow().bold());
                println!();
                println!("{}", actual);
            }
    ...
    fn run_file_mode(&self, runner: &TestRunner, input: &PathBuf, output: Option<&PathBuf>, expected: Option<&PathBuf>) -> Result<()> {
        println!("{} {:?}...", "Processing file".blue().bold(), input);
        
        let html_result = runner.process_file(input)
        ...
        if let Some(expected_path) = expected {
            ...
            let test_result = runner.test_string(&std::fs::read_to_string(input)?, Some(&expected_content));
    ...
    fn run_html_mode(&self, runner: &TestRunner, html: &str, spec_file: Option<&PathBuf>, example: Option<u32>) -> Result<()> {
        println!("{}", "Comparing HTML against specifications...".blue().bold());
        
        let spec_path = spec_file.cloned().unwrap_or_else(|| PathBuf::from("tests/spec/commonmark.json"));
        ...
        if let Some(example_num) = example {
            ...
            if let Some(test_case) = spec.find_by_example(example_num) {
                ...
                match result {
                    TestResult::Passed => {
                        println!("{} example {}", "✓ HTML matches".green().bold(), example_num);
                    }
                    TestResult::Failed { diff, .. } => {
                        println!("{} example {}", "✗ HTML differs from".red().bold(), example_num);
                        println!();
                        println!("{}", diff);
                    }
                ...
                return Err(anyhow::anyhow!("Example {} not found in specification", example_num));
            }
        } else {
            // Find closest match
            println!("{}", "Finding closest matches...".yellow());
            // This could be enhanced to find the most similar test cases
            println!("{}", "HTML comparison without specific example not yet implemented".yellow());
        }
    ...
    fn run_spec_mode(&self, runner: &TestRunner, file: Option<&PathBuf>, section: Option<&str>, example: Option<u32>, fail_fast: bool) -> Result<()> {
        ...
        for (test_case, result) in &filtered_results {
            match result {
                TestResult::Passed => {
                    if self.verbose {
                        println!("{} Example {} ({})", "✓".green(), test_case.example, test_case.section);
                    }
                }
                TestResult::Failed { diff, .. } => {
                    _failed_count += 1;
                    ...
                    if fail_fast {
                        return Err(anyhow::anyhow!("Stopping on first failure (fail-fast mode)"));
                    }
                }
                TestResult::NoBaseline { .. } => {
                    println!("{} Example {} ({}): No baseline", "?".yellow().bold(), test_case.example, test_case.section);
                }
                TestResult::Error { message } => {
                    println!("{} Example {} ({}): {}", "!".red().bold(), test_case.example, test_case.section, message);
