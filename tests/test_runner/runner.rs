...
impl TestRunner {
    ...
    pub fn process_markdown(&self, markdown: &str) -> Result<String, String> {
        if self.config.use_cache {
            ...
            global_parser_cache().render_with_cache(markdown, self.config.html_options.clone())
                .map_err(|e| format!("Failed to render HTML (cached): {}", e))
        } else {
            // Use direct non-cached processing for testing
            use marco::components::marco_engine::{parse_text, build_ast, render_html};
    ...
    pub fn run_test_case(&self, test_case: &TestCase) -> TestResult {
        let actual_html = match self.process_markdown(&test_case.markdown) {
            Ok(html) => html,
            Err(err) => return TestResult::Error {
    ...
    pub fn run_spec(&self, spec: &TestSpec) -> (Vec<(TestCase, TestResult)>, TestSummary) {
        ...
        for test_case in &spec.tests {
            if self.config.verbose {
                eprintln!("Running test example {} from section '{}'...", 
    ...
    pub fn run_spec_file<P: AsRef<Path>>(&self, path: P) -> Result<(Vec<(TestCase, TestResult)>, TestSummary)> {
        let spec = TestSpec::load_from_file(&path)
            .with_context(|| format!("Failed to load test specification from {:?}", path.as_ref()))?;
            
        if self.config.verbose {
            eprintln!("Loaded {} test cases from {}", spec.tests.len(), spec.source);
        }
    ...
    pub fn test_string(&self, markdown: &str, expected_html: Option<&str>) -> TestResult {
        ...
        match expected_html {
            ...
            None => TestResult::NoBaseline { actual: actual_html },
        }
    }
    ...
    pub fn run_all_tests(&self) -> Result<(Vec<(TestCase, TestResult)>, TestSummary)> {
        ...
        match self.run_commonmark_tests() {
            Ok((results, summary)) => {
                if self.config.verbose {
                    eprintln!("CommonMark tests: {} passed, {} failed, {} need baseline, {} errors",
        ...
        match self.run_marco_tests() {
            Ok((results, summary)) => {
                if self.config.verbose {
                    eprintln!("Marco tests: {} passed, {} failed, {} need baseline, {} errors",
                             summary.passed, summary.failed, summary.needs_baseline, summary.errors);
                }
                all_results.extend(results);
                combined_summary.total += summary.total;
...
mod tests {
    ...
    fn test_cached_vs_non_cached() {
        let cached_config = RunnerConfig { use_cache: true, ..RunnerConfig::default() };
        let non_cached_config = RunnerConfig { use_cache: false, ..RunnerConfig::default() };
