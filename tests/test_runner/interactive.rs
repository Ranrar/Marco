...
use crate::spec::{TestCase, TestResult, TestSpec};
...
impl InteractiveManager {
    ...
    fn add_new_test(&self) -> Result<()> {
        ...
        if self.confirm("Save this as a new test case?")? {
            ...
            let spec_path =
                if self.confirm("Add to Marco-specific tests? (otherwise CommonMark)")? {
        ...
        for (test_case, result) in results {
            ...
            } = result
            {
                ...
                if self.confirm("Update this test's expected result?")? {
                    ...
                    println!(
                        "{}",
                        "Baseline update functionality coming soon!"
                            .yellow()
                            .italic()
                    );
                }
    ...
    fn update_baselines(&self) -> Result<()> {
        ...
        println!(
            "{}",
            "This feature will allow updating existing test baselines."
                .yellow()
                .italic()
        );
                ...
                match TestSpec::load_from_file(spec_file) {
                    ...
                    Err(_) => {
                        println!(
                            "{}. {} (failed to load)",
                            i + 1,
                            spec_file.file_name().unwrap().to_string_lossy()
                        );
                    }
                }
            } else {
...
mod tests {
    ...
    fn test_interactive_manager_creation() {
        ...
        assert!(
            result.is_ok(),
            "Manager's runner should process markdown successfully"
        );
        let html = result.unwrap();
        assert!(
            html.contains("Test Header"),
            "HTML should contain the test header text"
        );
