...
mod parser_tests {
    ...
    use marco::components::marco_engine::{Rule, parse_to_html_cached};
    ...
    fn test_grammar_rule_validation_suite() {
        ...
        for (input, rule, should_succeed) in test_cases {
            ...
            if should_succeed {
                assert!(result.is_ok(), "Expected '{}' to parse successfully with rule {:?}", input, rule);
            } else {
                assert!(result.is_err(), "Expected '{}' to fail parsing with rule {:?}", input, rule);
            }
        }
    }
...
"#;
    ...
    fn test_grammar_rule(input: &str, rule: Rule, expected_success: bool) -> ParseResult<()> {
        ...
        match (result.is_ok(), expected_success) {
            (true, true) => Ok(()),
            (false, false) => Ok(()),
            (true, false) => Err(format!("Expected '{}' to fail with rule {:?}, but it succeeded", input, rule)),
            (false, true) => Err(format!("Expected '{}' to succeed with rule {:?}, but it failed: {}", 
                input, rule, result.unwrap_err())),
        }
    }
    ...
    fn test_parser_performance_with_parse_result() {
        ...
        if let Err(error_msg) = error_result {
            assert!(error_msg.contains("expected"), "Error message should be descriptive");
