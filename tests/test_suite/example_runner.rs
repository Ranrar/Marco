...
pub fn run_example_inspection(example_numbers: Vec<u32>) {
    ...
    println!("Inspecting {} example(s): {:?}\n", example_numbers.len(), example_numbers);
...
fn inspect_example(test: &CommonMarkTest) {
    ...
    println!("║  Example {} - {}", 
        test.example, 
        test.section.as_ref().unwrap_or(&"Unknown section".to_string())
    );
    println!("╚═══════════════════════════════════════════════════════════╝\n");
...
fn format_node_tree(node: &core::parser::ast::Node, prefix: &str, is_last: bool, output: &mut String) {
    ...
    let node_info = match &node.kind {
        NodeKind::Heading { level, text } => format!("Heading(level={}) \"{}\"", level, truncate(text, 40)),
        ...
        NodeKind::List { ordered, start, tight } => {
            format!("List({}ordered, tight={}, start={:?})", 
                if *ordered { "" } else { "un" }, tight, start)
        }
        ...
        NodeKind::Image { url, alt } => {
            format!("Image(url=\"{}\", alt=\"{}\")", 
                truncate(url, 20), truncate(alt, 20))
        }
