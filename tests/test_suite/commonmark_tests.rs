...
pub fn load_commonmark_tests() -> Vec<CommonMarkTest> {
    ...
    let raw_entries: Vec<RawCommonMarkEntry> =
...
pub fn load_extra_tests() -> Vec<CommonMarkTest> {
    ...
    let raw_entries: Vec<RawCommonMarkEntry> =
        serde_json::from_str(json).expect("Failed to parse extra.json");
    ...
    if let Some(section_filter) = section {
        let filtered: Vec<_> = tests
            ...
            .filter(|t| {
                t.section
                    .as_ref()
                    .map(|s| s.contains(&section_filter))
                    .unwrap_or(false)
            })
            .collect();
        ...
        for test in &filtered {
            ...
            let (passed, actual_html, error) =
                test_result.unwrap_or((false, None, Some("Panic".to_string())));
        ...
        let mut sections: Vec<_> = tests.iter().filter_map(|t| t.section.clone()).collect();
        sections.sort();
        sections.dedup();

        for section_name in &sections {
            let section_tests: Vec<_> = tests
                .iter()
        ...
        println!("─────────────────────────────────────────────────────────");
        println!(
            "Section Summary: {}/{} tests failed ({:.1}%)",
            total_failed, total_in_section, failed_percentage
        );
        println!(
            "Total progress: {}/{} CommonMark examples ({:.1}%)",
                            ...
                            match core::render::render(&document, &options) {
                                Ok(rendered_html) => {
                                    ...
                                    if !passed && verbose {
                                        println!(
                                            "❌ Example {} (lines {:?}-{:?})",
                                            test.example, test.start_line, test.end_line
                                        );
                                        println!(
                                            "   Input: {}",
                                        ...
                                        println!(
                                            "   Expected: {}",
                                            test.html.trim().replace("\n", "\\n")
                                        );
                                        println!(
                                            "   Actual:   {}",
                                            rendered_html.trim().replace("\n", "\\n")
                                        );
                                        println!();
                                    }
        ...
        for (section, passed, total) in &section_results {
            ...
            println!(
                "  {} {} - {}/{} ({:.1}%)",
                status, section, passed, total, percentage
            );
        }
        ...
        println!("\n─────────────────────────────────────────────────────────");
        println!(
            "Total: {} sections with {} examples",
            sections.len(),
            total_tested
        );
        println!(
            "CommonMark coverage: {}/{} examples ({:.1}%)",
...
pub fn run_extra_tests() {
    ...
    if failed > 0 {
        println!(
            "Extra tests: {}/{} passed ({:.1}%)",
            passed, total_tests, coverage_percentage
        );
        println!("Failed examples: {:?}", failed_examples);
    } else {
        println!(
            "✓ All extra tests passed: {}/{} ({:.1}%)",
            passed, total_tests, coverage_percentage
        );
    }
    println!("─────────────────────────────────────────────────────────");
