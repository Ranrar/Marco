...
fn is_valid_nested_highlight(h1: &core::lsp::Highlight, h2: &core::lsp::Highlight) -> bool {
    ...
    let h1_contains_h2 = (h1.span.start.line < h2.span.start.line || 
                          (h1.span.start.line == h2.span.start.line && h1.span.start.column <= h2.span.start.column)) &&
    ...
    let h2_contains_h1 = (h2.span.start.line < h1.span.start.line || 
                          (h2.span.start.line == h1.span.start.line && h2.span.start.column <= h1.span.start.column)) &&
            ...
            Ok(document) => {
                ...
                for i in 0..highlights.len() {
                    ...
                    for j in (i + 1)..highlights.len() {
                        ...
                        // Check for overlap
                        let overlap = if h1.span.start.line == h2.span.start.line && h1.span.end.line == h2.span.end.line {
                        ...
                        if overlap {
                            ...
                            if !is_valid_nesting {
                                failed += 1;
                                failed_examples.push((test.example, format!("Invalid overlapping highlights: {:?} vs {:?}", h1.tag, h2.tag)));
                                has_error = true;
                                break;
                            }
                        }
