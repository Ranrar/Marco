    ...
    match parse(markdown) {
        Ok(document) => {
            ...
            for (i, highlight) in highlights.iter().enumerate() {
                ...
                let _gtk_end_line = end.line - 1;

                // For single-line, calculate char offsets
                if start.line == end.line {
                    if let Some(line) = lines.get(gtk_start_line) {
                        let start_byte_idx = start.column - 1;
                        let end_byte_idx = end.column - 1;

                        let start_char_offset: usize = line
                            .char_indices()
                            .take_while(|(byte_idx, _)| *byte_idx < start_byte_idx)
                            .count();

                        let end_char_offset: usize = line
                            .char_indices()
        ...
        Ok(document) => {
            ...
            for h in highlights {
                println!(
                    ...
                    h.span.end.line,
                    h.span.end.column,
                    &markdown[(h.span.start.column - 1)..(h.span.end.column - 1)]
                );
            }
        }
