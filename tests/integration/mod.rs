...
fn test_marco_engine_basic_markdown() {
    ...
    assert!(result.is_ok(), "Marco engine should successfully process basic markdown");
    
    let html = result.unwrap();
    assert!(html.contains("Hello World"), "HTML should contain the header text");
...
fn test_marco_engine_empty_input() {
    ...
    assert!(result.is_ok(), "Marco engine should handle empty input gracefully");
}
...
fn test_marco_engine_code_blocks() {
    ...
    let html = result.unwrap();
    assert!(html.contains("main"), "HTML should contain the code content");
}
...
fn test_commonmark_spec_file_exists() {
    ...
    assert!(spec_path.exists(), "CommonMark specification file should exist");
...
mod spec_tests {
    ...
    fn run_commonmark_spec_tests() {
        ...
        let test_cases: Vec<serde_json::Value> = serde_json::from_str(&content)
            .expect("Should parse CommonMark spec");
                ...
                match parse_to_html_cached(markdown) {
                    Ok(actual_html) => {
                        ...
                        if normalize_html(&actual_html) == normalize_html(expected_html) {
                            ...
                            failed += 1;
                            if failed <= 5 { // Only show first few failures to avoid spam
        ...
        eprintln!("CommonMark spec results: {} passed, {} failed", passed, failed);
...
fn benchmark_marco_performance() {
    ...
    println!("Processed {} iterations in {:?} (avg: {:?} per iteration)", 
             100, duration, duration / 100);
    
    // Just verify it doesn't take an unreasonably long time
    assert!(duration.as_secs() < 10, "Performance test should complete within 10 seconds");
}
...
fn test_marco_parser_cache() {
    ...
    assert_eq!(result1.unwrap(), result2.unwrap(), "Cached result should be identical");
