...
fn test_html_span_not_autolink() {
    ...
    if let NodeKind::Paragraph = &para.kind {
        ...
        let has_inline_html = inlines.iter().any(|n| matches!(n.kind, NodeKind::InlineHtml(_)));
...
fn test_highlighting_html_vs_autolink() {
    ...
    let has_html_highlight = html_highlights.iter().any(|h| {
        matches!(h.tag, HighlightTag::InlineHtml)
    });
    let has_link_highlight = html_highlights.iter().any(|h| {
        matches!(h.tag, HighlightTag::Link)
    });
    
    assert!(has_html_highlight, "HTML tag should have InlineHtml highlight");
    assert!(!has_link_highlight, "HTML tag should NOT have Link highlight");
    ...
    let has_html_highlight = link_highlights.iter().any(|h| {
        matches!(h.tag, HighlightTag::InlineHtml)
    });
    let has_link_highlight = link_highlights.iter().any(|h| {
        matches!(h.tag, HighlightTag::Link)
    });
    
    assert!(!has_html_highlight, "Autolink should NOT have InlineHtml highlight");
...
fn test_mixed_html_and_autolinks() {
    ...
    if let NodeKind::Paragraph = &para.kind {
        ...
        let has_inline_html = inlines.iter().any(|n| matches!(n.kind, NodeKind::InlineHtml(_)));
        let has_link = inlines.iter().any(|n| matches!(n.kind, NodeKind::Link { .. }));
