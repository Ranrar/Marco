...
fn test_gfm_task_list_items_parse_with_task_checkbox_marker() {
	...
	for (idx, li) in list.children.iter().enumerate() {
		...
		assert!(matches!(first.kind, NodeKind::TaskCheckbox { .. }), "item {} missing TaskCheckbox", idx);
	}
}
...
fn test_gfm_task_list_renders_svg_checkbox_icons() {
	...
	assert!(html.contains("task-list-item-checkbox"), "expected checkbox span class");
	assert!(!html.contains("[ ]"), "marker should be stripped from output");
	assert!(!html.contains("[x]"), "marker should be stripped from output");
}
...
fn test_task_checkbox_marker_without_list_still_renders_svg() {
	...
	assert!(doc.children.iter().all(|n| !matches!(n.kind, NodeKind::List { .. })));

	// Should contain inline checkbox nodes.
	let mut inline_count = 0usize;
	for p in &doc.children {
		if matches!(p.kind, NodeKind::Paragraph)
			&& matches!(p.children.first().map(|c| &c.kind), Some(NodeKind::TaskCheckboxInline { .. }))
		{
			inline_count += 1;
		}
	}
...
[X] Optimize parser performance and caching\n";
	...
	let html = core::render::render(&doc, &core::render::RenderOptions::default()).expect("render failed");
	assert!(html.contains("marco-task-icon"), "expected svg icon class");
	assert!(!html.contains("[ ]"), "marker should be stripped from output");
	assert!(!html.contains("[x]"), "marker should be stripped from output");
	assert!(!html.contains("[X]"), "marker should be stripped from output");
