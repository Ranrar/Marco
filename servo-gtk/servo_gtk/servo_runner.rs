...
impl ServoRunner {
    pub fn new() -> Self {
        ...
        // Use servo-runner directly (production mode) instead of `cargo run`
        let subprocess = launcher
            .spawn(&[OsStr::new("servo-runner")])
        ...
        // Log subprocess PID for debugging
        if let Some(pid) = subprocess.identifier() {
            debug!("servo-runner subprocess spawned with ID: {}", pid);
        }
...
impl Drop for ServoRunner {
    fn drop(&mut self) {
        info!("ServoRunner Drop impl called");
        // Send shutdown message
        self.shutdown();
        
        // Force terminate subprocess after shutdown message
        // If the subprocess doesn't exit gracefully, we force kill it
        if let Some(pid) = self._subprocess.identifier() {
            info!("Cleaning up servo-runner subprocess (PID: {})", pid);
        }
        
        // Force exit to ensure subprocess is killed (sends SIGKILL on Unix)
        self._subprocess.force_exit();
        info!("force_exit() called on subprocess");
    }
}
