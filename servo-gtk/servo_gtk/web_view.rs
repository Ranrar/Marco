...
mod imp {
    ...
    impl ObjectImpl for WebView {
        ...
        fn dispose(&self) {
            info!("WebView dispose called - cleaning up ServoRunner");
            // Take the ServoRunner out of the RefCell to trigger its Drop implementation
            // This ensures force_exit() is called on the subprocess
            if let Some(servo) = self.servo_runner.take() {
                info!("Calling shutdown on ServoRunner");
                servo.shutdown();
                // Servo runner dropped here, triggering force_exit() in Drop impl
                info!("ServoRunner dropped - force_exit should have been called");
            } else {
                info!("ServoRunner already cleaned up");
            }
        }
    }
...
impl WebView {
    ...
    /// Force cleanup of ServoRunner subprocess
    /// Call this explicitly before dropping the WebView to ensure servo-runner is killed
    pub fn cleanup(&self) {
        info!("WebView::cleanup() called explicitly");
        let imp = self.imp();
        if let Some(servo) = imp.servo_runner.take() {
            info!("Taking ServoRunner from RefCell and dropping it");
            servo.shutdown();
            // ServoRunner drops here, triggering force_exit()
            drop(servo);
            info!("ServoRunner explicitly dropped");
        } else {
            info!("ServoRunner already cleaned up");
        }
    }
