...
fn main() {
    ...
    // Full pipeline golden tests: markdown -> block AST -> HTML render
    let golden_html_out_path = "tests/commonmark_golden.rs";
    let mut golden_html_out = String::new();
    golden_html_out.push_str("// This file is generated by build.rs\n");
    golden_html_out.push_str("use marco::logic::core::block::parser::parse_blocks;\n");
    golden_html_out.push_str("use marco::logic::ast::blocks_and_inlines::{Block, ContainerBlock};\n");
    golden_html_out.push_str("use marco::logic::renderer::html_render::HtmlRenderer;\n");
    golden_html_out.push_str("use marco::logic::core::block::blocknode_to_block;\n"); // Add helper import
    golden_html_out.push_str("#[cfg(test)]\nmod golden_html_spec_generated {\n");
    golden_html_out.push_str("    use super::*;\n");
    if let Some(arr) = json.as_array() {
        for (i, obj) in arr.iter().enumerate() {
            let markdown = obj.get("markdown").and_then(|v| v.as_str()).unwrap_or("");
            let html = obj.get("html").and_then(|v| v.as_str()).unwrap_or("");
            ...
            golden_html_out.push_str(&format!("    #[test]\n    fn golden_html_example_{}() {{\n", i + 1));
            golden_html_out.push_str(&format!("        let block_nodes = parse_blocks(r#\"{}\"#);\n", markdown.replace("\"", "\\\"")));
            golden_html_out.push_str("        let blocks: Vec<Block> = block_nodes.into_iter().map(|node| blocknode_to_block(&node)).collect();\n");
            golden_html_out.push_str("        let doc = Block::Container(ContainerBlock::Document(blocks, None));\n");
            golden_html_out.push_str("        let rendered = HtmlRenderer::render_document(&doc);\n");
            golden_html_out.push_str(&format!("        assert_eq!(rendered.trim(), r###\"{}\"###.trim(), \"HTML render mismatch for example {}\");\n", html, i + 1));
            golden_html_out.push_str("    }\n");
        }
    }
    golden_html_out.push_str("}\n");
    fs::write(golden_html_out_path, golden_html_out).expect("Failed to write generated golden HTML test file");
}
