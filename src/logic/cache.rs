...
use std::sync::{Arc, RwLock, OnceLock, Weak};
...
impl SimpleFileCache {
        ...
        T: Send + Sync + 'static,
    {
        ...
        if let Ok(mut open_files) = self.open_files.write() {
            ...
            open_files.entry(path).or_insert_with(Vec::new).push(weak_ref);
        }
    }
    ...
    pub fn search_files_fast<P: AsRef<Path>>(&self, path: P, pattern: Option<&str>) -> Result<(Vec<PathBuf>, Vec<PathBuf>)> {
        let path = path.as_ref().to_path_buf();
            ...
            if let Ok(cache) = self.directory_cache.read() {
                if let Some(entry) = cache.get(&path) {
                    if entry.is_fresh() {
                        ...
                        return Ok(self.apply_search_pattern(&entry.files, &entry.directories, pattern));
                    }
                }
            }
        }
    ...
    fn apply_search_pattern(&self, files: &[PathBuf], directories: &[PathBuf], pattern: Option<&str>) -> (Vec<PathBuf>, Vec<PathBuf>) {
        match pattern {
            Some(search_pattern) if !search_pattern.is_empty() => {
                ...
                let filtered_files = files.iter()
                    .filter(|path| {
                        if let Some(name) = path.file_name() {
                            name.to_string_lossy().to_lowercase().contains(&pattern_lower)
                        } else {
                            false
                        }
                    })
                ...
                let filtered_dirs = directories.iter()
                    .filter(|path| {
                        if let Some(name) = path.file_name() {
                            name.to_string_lossy().to_lowercase().contains(&pattern_lower)
                        } else {
    ...
    pub fn create_file_monitor(path: &Path, content_cache: Arc<RwLock<HashMap<PathBuf, CachedFile>>>) -> Result<gio::FileMonitor> {
        ...
        match file.monitor_file(gio::FileMonitorFlags::NONE, gio::Cancellable::NONE) {
                            ...
                            if let Some(changed_path) = file_obj.path() {
                                if let Ok(mut cache) = content_cache.write() {
                                    if cache.remove(&changed_path).is_some() {
                                        log::debug!("Cache invalidated due to external change: {}", 
            ...
            Err(e) => {
                Err(anyhow::anyhow!("Failed to create file monitor: {}", e))
            }
        }
    }
