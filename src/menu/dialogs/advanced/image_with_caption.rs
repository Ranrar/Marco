...
pub fn show_image_with_caption_dialog(window: &gtk4::Window, editor: &editor::MarkdownEditor) {
    ...
    // Help text
    let help_label = Label::new(Some("Creates a figure with caption using HTML structure for better semantic markup."));
    ...
    caption_entry.connect_changed({let update_preview = update_preview.clone(); move |_| update_preview()});
