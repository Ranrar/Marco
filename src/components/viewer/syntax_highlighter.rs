...
impl SyntaxHighlighter {
    ...
    pub fn new() -> Result<Self> {
        ...
        let light_theme = theme_set
            ...
            .with_context(|| format!("Light theme '{}' not found and no fallbacks available", LIGHT_THEME_NAME))?
            .clone();
        ...
        let dark_theme = theme_set
            ...
            .or_else(|| theme_set.themes.values().next()) // final fallback to any theme
            .with_context(|| format!("Dark theme '{}' not found and no fallbacks available", DARK_THEME_NAME))?
            .clone();

        log::debug!("[syntax_highlighter] Loaded themes: light='{}' ({}), dark='{}' ({})", 
                   LIGHT_THEME_NAME,
    ...
    pub fn highlight_to_html(&self, code: &str, language: &str, _theme_mode: &str) -> Result<String> {
    ...
    pub fn generate_css(&mut self, theme_mode: &str) -> String {
        ...
        if self.css_cache.len() < 10 {
            ...
            log::warn!("[syntax_highlighter] CSS cache limit reached, not caching theme_mode: {}", theme_mode);
        }
        
        log::debug!("[syntax_highlighter] Generated CSS for theme_mode: {}", theme_mode);
        css
    }
...
pub fn generate_css_with_global(theme_mode: &str) -> Result<String> {
    ...
    SYNTAX_HIGHLIGHTER.with(|highlighter| {
        let mut h = highlighter.borrow_mut();
        let syntax_highlighter = h.as_mut()
...
mod tests {
    ...
    fn smoke_test_syntax_highlighter_creation() {
        ...
        assert!(result.is_ok(), "Should be able to create syntax highlighter");
