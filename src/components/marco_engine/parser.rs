...
/// Simple parse result type - just use Result directly
pub type ParseResult<T> = Result<T, crate::components::marco_engine::errors::MarcoError>;

/// Convenience function: Parse document with Rule::document
pub fn parse_document(input: &str) -> ParseResult<Pairs<'_, Rule>> {
    MarcoParser::parse(Rule::document, input).map_err(|e| {
        crate::components::marco_engine::errors::MarcoError::parse_error(e.to_string())
    })
}

/// Convenience function: Parse with specific rule
pub fn parse_with_rule(input: &str, rule: Rule) -> ParseResult<Pairs<'_, Rule>> {
    MarcoParser::parse(rule, input).map_err(|e| {
        crate::components::marco_engine::errors::MarcoError::parse_error(e.to_string())
    })
}
