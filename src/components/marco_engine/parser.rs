...
impl MarkdownSyntaxMap {
    ...
    pub fn load_active_schema(_settings_path: &str, _schema_root: &str) -> Result<Option<Self>, Box<dyn std::error::Error>> {
        // Create a basic syntax map for now
        let mut rules = HashMap::new();
        
        rules.insert("heading".to_string(), SyntaxRule {
            name: "heading".to_string(),
            pattern: "^#{1,6} .+".to_string(),
            description: "Heading".to_string(),
        });
        
        rules.insert("paragraph".to_string(), SyntaxRule {
...
pub fn parse_document_blocks(input: &str, _syntax_map: &MarkdownSyntaxMap) -> (Vec<BlockElement>, Vec<(String, String)>) {
    // Simple implementation for now - just identify basic block types
    let mut blocks = Vec::new();
    let link_defs = Vec::new(); // Empty for now - could be populated with actual link definitions later
...
fn parse_pair(pair: pest::iterators::Pair<Rule>) -> Option<Node> {
    match pair.as_rule() {
        ...
        Rule::block => {
            // Block is just a wrapper, process its children
            for inner_pair in pair.into_inner() {
                return parse_pair(inner_pair);
            }
            None
        }
        ...
        Rule::frontmatter | Rule::yaml_frontmatter | Rule::toml_frontmatter => {
            let content = pair.as_str();
            let format = if content.starts_with("---") { "yaml" } else { "toml" };
        ...
        Rule::math_block => {
            let content = pair.as_str().trim_start_matches("$$").trim_end_matches("$$").trim();
        ...
        Rule::html_block | Rule::html_tag_block | Rule::html_comment_block | Rule::html_cdata_block => {
        ...
        Rule::definition => {
            // Process the inner definition type
            for inner_pair in pair.into_inner() {
                return parse_pair(inner_pair);
            }
            None
    ...
    Rule::paragraph_content | Rule::inline_content => {
            ...
            // For paragraph and inline content, process all children
            ...
            // If we have multiple nodes, we need to collect them
            // For now, return the first one found (this needs more sophisticated handling)
            if let Some(first_node) = nodes.into_iter().next() {
                Some(first_node)
            } else {
                // If no special elements, treat as plain text
