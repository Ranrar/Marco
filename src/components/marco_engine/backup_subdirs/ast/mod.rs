...
// Simple AstBuilder - will be implemented directly here for now
...
impl AstBuilder {
    ...
    pub fn build(pairs: Pairs<Rule>) -> Result<Node, MarcoError> {
        ...
        let span = node::Span::new(0, 0, 1, 1);
    ...
    fn span_from_pair(pair: &pest::iterators::Pair<Rule>) -> node::Span {
        let span = pair.as_span();
        let start = span.start() as u32;
        let end = span.end() as u32;
        node::Span::new(start, end, 1, 1) // Line/column calculation simplified for now
    }
