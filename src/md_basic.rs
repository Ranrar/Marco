...
impl MarkdownParser {
    ...
    pub fn to_html(&self, markdown: &str) -> String {
        ...
        while i < lines.len() {
            ...
            if let Some(heading_html) = self.parse_heading(line) {
                self.close_all_blocks(&mut html, &mut in_unordered_list, &mut in_ordered_list, &mut in_blockquote);
    ...
    fn close_all_blocks(&self, html: &mut String, in_ul: &mut bool, in_ol: &mut bool, in_bq: &mut bool) {
    ...
    pub fn parse_heading(&self, line: &str) -> Option<String> {
        if let Some(captures) = self.heading_regex.captures(line) {
            ...
            return Some(format!("<h{}>{}</h{}>", level, self.process_inline_formatting(content), level));
        }
        None
    }
    ...
    pub fn process_inline_formatting(&self, text: &str) -> String {
        ...
        result = self.image_regex.replace_all(&result, |caps: &regex::Captures| {
            format!("<img src=\"{}\" alt=\"{}\">", &caps[2], &caps[1])
        }).to_string();
        
        // Links
        result = self.link_regex.replace_all(&result, |caps: &regex::Captures| {
        ...
        // Inline code
        result = self.code_regex.replace_all(&result, "<code>$1</code>").to_string();
    ...
    fn process_bold_italic(&self, text: &str) -> String {
        ...
        result = bold_italic_regex.replace_all(&result, "<strong><em>$1</em></strong>").to_string();
        
        // Handle ___text___ (bold italic with underscores)
        let bold_italic_underscore_regex = Regex::new(r"___([^_]+)___").unwrap();
        result = bold_italic_underscore_regex.replace_all(&result, "<strong><em>$1</em></strong>").to_string();
        
        // Bold with ** (must come before italic *)
        result = self.bold_double_regex.replace_all(&result, "<strong>$1</strong>").to_string();
        
        // Bold with __
        result = self.bold_underscore_regex.replace_all(&result, "<strong>$1</strong>").to_string();
        
        // Italic with * (only if not part of **)
        result = self.italic_single_regex.replace_all(&result, "<em>$1</em>").to_string();
        
        // Italic with _ (only if not part of __)
        result = self.italic_underscore_regex.replace_all(&result, "<em>$1</em>").to_string();
        
        // Strikethrough with ~~
        result = self.strikethrough_regex.replace_all(&result, "<del>$1</del>").to_string();
    ...
    pub fn parse_bold(&self, text: &str) -> String {
        let mut result = self.bold_double_regex.replace_all(text, "<strong>$1</strong>").to_string();
        result = self.bold_underscore_regex.replace_all(&result, "<strong>$1</strong>").to_string();
        result
    }
    ...
    pub fn parse_italic(&self, text: &str) -> String {
        let mut result = self.italic_single_regex.replace_all(text, "<em>$1</em>").to_string();
        result = self.italic_underscore_regex.replace_all(&result, "<em>$1</em>").to_string();
        result
    }
    ...
    pub fn parse_inline_code(&self, text: &str) -> String {
        self.code_regex.replace_all(text, "<code>$1</code>").to_string()
    }

    #[allow(dead_code)]
    pub fn parse_links(&self, text: &str) -> String {
        self.link_regex.replace_all(text, |caps: &regex::Captures| {
            format!("<a href=\"{}\">{}</a>", &caps[2], &caps[1])
        }).to_string()
    }
    ...
    pub fn parse_images(&self, text: &str) -> String {
        self.image_regex.replace_all(text, |caps: &regex::Captures| {
            format!("<img src=\"{}\" alt=\"{}\">", &caps[2], &caps[1])
        }).to_string()
    }
    ...
    pub fn parse_strikethrough(&self, text: &str) -> String {
        self.strikethrough_regex.replace_all(text, "<del>$1</del>").to_string()
    }
}
...
mod tests {
    ...
    fn test_bold_both_syntaxes() {
        ...
        assert_eq!(parser.parse_bold("I love __bold text__."), "I love <strong>bold text</strong>.");
    }
    ...
    fn test_italic_both_syntaxes() {
        ...
        assert_eq!(parser.parse_italic("This is _italic text_."), "This is <em>italic text</em>.");
    }
