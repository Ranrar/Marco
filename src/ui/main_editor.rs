...
pub fn create_editor_with_preview(
    ...
    let (editor_widget, buffer, source_view, scrolled_css_provider) = render_editor_with_view(&scheme_id, style_scheme.as_ref(), &font_family, font_size_pt);
    ...
    let mut css = fs::read_to_string(&css_path).unwrap_or_else(|_| String::from("body { background: #fff; color: #222; }"));
    ...
    let scroll_scale: f64 = std::env::var("MARCO_SCROLL_SCALE").ok().and_then(|s| s.parse::<f64>().ok()).unwrap_or(1.0);
    let wheel_js = format!(r#"<script>
    (function(){{
        const scale = {scale};
    ...
    let wheel_js_for_signal = wheel_js_rc.clone();
        let preview_timeout_id: Rc<std::cell::Cell<Option<glib::SourceId>>> = Rc::new(std::cell::Cell::new(None));
        ...
        buffer_for_signal.connect_changed(move |buf| {
            ...
            let id = glib::timeout_add_local(std::time::Duration::from_millis(400), move || {
                ...
                let mut html_body_with_js = html_body.clone();
                html_body_with_js.push_str(&*wheel_js_local);
                let html = wrap_html_document(&html_body_with_js, &css_clone_inner.borrow(), &theme_mode_inner.borrow());
    ...
    let update_theme = Box::new(move |scheme_id: &str| {
            if let Some(scheme) = theme_manager_for_scheme.borrow().get_editor_scheme(scheme_id) {
    ...
    "#, thumb = thumb, track = track);
    let _ = scrolled_provider_for_update.load_from_data(&new_css);
    ...
    let wheel_js_local = wheel_js_for_update.clone();
        glib::idle_add_local(move || {
            let text = buffer_clone_for_load.text(&buffer_clone_for_load.start_iter(), &buffer_clone_for_load.end_iter(), false).to_string();
            let mut html_body = markdown_to_html(&text, &*markdown_opts_clone_for_load);
            html_body.push_str(&*wheel_js_local);
            let html = wrap_html_document(&html_body, &css_for_load.borrow(), &theme_mode_clone_for_load.borrow());
...
pub fn render_editor_with_view(
    ...
    font_family: &str,
    font_size_pt: f64
) -> (gtk4::Box, sourceview5::Buffer, sourceview5::View, gtk4::CssProvider) {
    ...
    let _ = font_provider.load_from_data(&css);
    source_view.style_context().add_provider(&font_provider, gtk4::STYLE_PROVIDER_PRIORITY_APPLICATION);
    ...
    if let Some(display) = gtk4::gdk::Display::default() {
        gtk4::style_context_add_provider_for_display(&display, &scrolled_provider, gtk4::STYLE_PROVIDER_PRIORITY_APPLICATION);
    }
