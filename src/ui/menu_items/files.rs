use gtk4::{
    prelude::*, FileChooserAction, FileChooserNative, ResponseType, 
    MessageDialog, DialogFlags, MessageType, ButtonsType, Window
};
...
impl FileDialogs {
    ...
    pub async fn show_save_changes_dialog<W: IsA<Window>>(
        ...
        dialog.set_secondary_text(Some("Your changes will be lost if you don't save them."));
        ...
        let result = match response {
            ResponseType::Yes => SaveChangesResult::Save,
            ResponseType::No => SaveChangesResult::Discard,
    ...
    /// ).await;
    /// ```
    pub async fn show_info_dialog<W: IsA<Window>>(
        parent: &W,
        title: &str,
        message: &str,
    ) {
