...
impl MarkdownEditor {
    ...
    fn show_text_selection_required_dialog(&self, parent: &gtk4::Window, feature_name: &str) {
        ...
        let message = format!(
            "Please select text in the editor before applying {} formatting.",
            feature_name
        );
    ...
    fn toggle_format_selection_only(&self, prefix: &str, suffix: &str) {
        ...
        println!(
            "DEBUG: toggle_format_selection_only called with prefix='{}', suffix='{}'",
            prefix, suffix
        );
        ...
        let (actual_start, actual_end, text) =
            if let Some((expanded_start, expanded_end, expanded_text)) =
                self.expand_selection_to_include_formatting()
            {
                println!(
                    "DEBUG: Expanded selection to include formatting: '{}'",
                    expanded_text
                );
                ...
                let text = gtk_buffer
                    .text(&original_start, &original_end, false)
                    .to_string();
        ...
        println!(
            "DEBUG: Completed formatting, has_selection: {}",
        ...
        glib::idle_add_local_once(move || {
            ...
            if gtk_buffer_clone.has_selection() {
                ...
                let new_start = gtk_buffer_clone
                    .iter_at_offset(cursor_offset - new_text.chars().count() as i32);
    ...
    fn toggle_format_selection_only_with_dialog(
        &self,
        prefix: &str,
        suffix: &str,
        feature_name: &str,
        ...
        if gtk_buffer.has_selection() {
            ...
            println!(
                "DEBUG: Selected text before formatting: '{}'",
                selected_text
            );
            ...
            *self.last_formatting_action.borrow_mut() = Some(std::time::Instant::now());
            ...
            glib::timeout_add_local_once(std::time::Duration::from_millis(1), move || {
                ...
                if gtk_buffer_clone.has_selection() {
                    ...
                    println!(
                        "DEBUG: Selection maintained after 1ms delay: '{}'",
                        new_selected_text
                    );
                } else {
            ...
            glib::timeout_add_local_once(std::time::Duration::from_millis(10), move || {
                ...
                if gtk_buffer_clone2.has_selection() {
                    ...
                    println!(
                        "DEBUG: Final selection check (10ms): '{}'",
                        new_selected_text
                    );
                } else {
    ...
    fn expand_selection_to_include_formatting(
        &self,
        ...
        for _ in 0..10 {
            // Look up to 10 characters forward
            if !expanded_end.forward_char() {
                break;
            }
        }

        let expanded_text = gtk_buffer
            .text(&expanded_start, &expanded_end, false)
            .to_string();
    ...
    pub fn is_cursor_in_format(&self, prefix: &str, suffix: &str) -> bool {
        ...
        let line_start = gtk_buffer
            .iter_at_line(cursor_iter.line())
            .unwrap_or_else(|| cursor_iter);
        let mut line_end = gtk_buffer
            .iter_at_line(cursor_iter.line())
            .unwrap_or_else(|| cursor_iter);
        ...
        self.find_format_at_cursor(&line_text, cursor_offset, prefix, suffix)
            .is_some()
    }
        ...
        if existing_formats.is_empty() {
            ...
            for (prefix, suffix) in &existing_formats {
                if inner_text.starts_with(prefix) && inner_text.ends_with(suffix) {
                    inner_text =
                        inner_text[prefix.len()..inner_text.len() - suffix.len()].to_string();
                }
            }
